...
func TestLabelerRuleTTL(t *testing.T) {
	...
	re.NoError(failpoint.Enable("github.com/tikv/pdv2/2/server/schedule/labeler/regionLabelExpireSub1Minute", "return(true)"))

	// rule2 should expire and only 2 labels left.
	labels := labeler.GetRegionLabels(region)
	re.Len(labels, 2)

	re.NoError(failpoint.Disable("github.com/tikv/pdv2/2/server/schedule/labeler/regionLabelExpireSub1Minute"))
