...
func TestLoadRegionsExceedRangeLimit(t *testing.T) {
	re := require.New(t)
	re.NoError(failpoint.Enable("github.com/tikv/pdv2/2/server/storage/kv/withRangeLimit", "return(500)"))
	storage := NewStorageWithMemoryBackend()
	cache := core.NewRegionsInfo()
	...
	re.NoError(failpoint.Disable("github.com/tikv/pdv2/2/server/storage/kv/withRangeLimit"))
}
