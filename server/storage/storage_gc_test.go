...
func TestLoadMinServiceSafePoint(t *testing.T) {
	...
	re.NoError(failpoint.Enable("github.com/tikv/pdv2/2/server/storage/endpoint/removeExpiredKeys", "return(true)"))
	...
	re.NoError(err)
	re.Nil(ssp)
	re.NoError(failpoint.Disable("github.com/tikv/pdv2/2/server/storage/endpoint/removeExpiredKeys"))
}
