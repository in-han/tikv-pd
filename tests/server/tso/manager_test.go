...
func TestNextLeaderKey(t *testing.T) {
	...
	defer cluster.Destroy()
	re.NoError(err)
	re.NoError(failpoint.Enable("github.com/tikv/pdv9/server/tso/injectNextLeaderKey", "return(true)"))
	re.NoError(cluster.RunInitialServers())
	...
	re.Equal("", originName)
	re.NoError(failpoint.Disable("github.com/tikv/pdv9/server/tso/injectNextLeaderKey"))
	cluster.CheckClusterDCLocation()
