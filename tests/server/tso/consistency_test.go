...
func (suite *tsoConsistencyTestSuite) TestSynchronizedGlobalTSOOverflow() {
	...
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	suite.NoError(failpoint.Enable("github.com/tikv/pdv2/server/tso/globalTSOOverflow", `return(true)`))
	suite.getTimestampByDC(ctx, cluster, tso.GlobalDCLocation)
	suite.NoError(failpoint.Disable("github.com/tikv/pdv2/server/tso/globalTSOOverflow"))
}

func (suite *tsoConsistencyTestSuite) TestLocalAllocatorLeaderChange() {
	suite.NoError(failpoint.Enable("github.com/tikv/pdv2/server/mockLocalAllocatorLeaderChange", `return(true)`))
	dcLocationConfig := map[string]string{
		"pd1": "dc-1",
	}
	dcLocationNum := len(dcLocationConfig)
	...
	defer cancel()
	suite.getTimestampByDC(ctx, cluster, tso.GlobalDCLocation)
	suite.NoError(failpoint.Disable("github.com/tikv/pdv2/server/mockLocalAllocatorLeaderChange"))
}
...
func (suite *tsoConsistencyTestSuite) TestLocalTSOAfterMemberChanged() {
	...
	suite.NoError(failpoint.Enable("github.com/tikv/pdv2/server/tso/systemTimeSlow", `return(true)`))
	...
	suite.NoError(failpoint.Disable("github.com/tikv/pdv2/server/tso/systemTimeSlow"))
}
...
func TestFallbackTSOConsistency(t *testing.T) {
	...
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	re.NoError(failpoint.Enable("github.com/tikv/pdv2/server/tso/fallBackSync", `return(true)`))
	re.NoError(failpoint.Enable("github.com/tikv/pdv2/server/tso/fallBackUpdate", `return(true)`))
	var err error
	...
	svr.Close()
	re.NoError(failpoint.Disable("github.com/tikv/pdv2/server/tso/fallBackSync"))
	re.NoError(failpoint.Disable("github.com/tikv/pdv2/server/tso/fallBackUpdate"))
	re.NoError(svr.Run())
	cluster.WaitLeader()
	var wg sync.WaitGroup
	wg.Add(tsoRequestConcurrencyNumber)
