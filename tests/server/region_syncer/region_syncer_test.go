...
func TestRegionSyncer(t *testing.T) {
	...
	defer cancel()
	re.NoError(failpoint.Enable("github.com/tikv/pdv9/server/storage/regionStorageFastFlush", `return(true)`))
	re.NoError(failpoint.Enable("github.com/tikv/pdv9/server/syncer/noFastExitSync", `return(true)`))
	...
	re.NoError(failpoint.Disable("github.com/tikv/pdv9/server/syncer/noFastExitSync"))
	re.NoError(failpoint.Disable("github.com/tikv/pdv9/server/storage/regionStorageFastFlush"))
}
...
func TestPrepareChecker(t *testing.T) {
	...
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()
	re.NoError(failpoint.Enable("github.com/tikv/pdv9/server/cluster/changeCoordinatorTicker", `return(true)`))
	cluster, err := tests.NewTestCluster(ctx, 1, func(conf *config.Config, serverName string) { conf.PDServerCfg.UseRegionStorage = true })
	defer cluster.Destroy()
	re.NoError(err)
	...
	re.NoError(failpoint.Disable("github.com/tikv/pdv9/server/cluster/changeCoordinatorTicker"))
}
