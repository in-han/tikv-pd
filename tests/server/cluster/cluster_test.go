...
import (
	...
	"github.com/tikv/pdv9/pkg/dashboard"
	...
	"github.com/tikv/pdv9/server/id"
	syncer "github.com/tikv/pdv9/server/region_syncer"
...
func TestRaftClusterMultipleRestart(t *testing.T) {
	...
	re.NoError(failpoint.Enable("github.com/tikv/pdv9/server/cluster/highFrequencyClusterJobs", `return(true)`))
	for i := 0; i < 100; i++ {
		err = rc.Start(leaderServer.GetServer())
		re.NoError(err)
	...
	re.NoError(failpoint.Disable("github.com/tikv/pdv9/server/cluster/highFrequencyClusterJobs"))
}
...
func TestStoreVersionChange(t *testing.T) {
	...
	re.NoError(failpoint.Enable("github.com/tikv/pdv9/server/versionChangeConcurrency", `return(true)`))
	wg.Add(1)
	go func() {
		defer wg.Done()
		_, err = putStore(grpcPDClient, clusterID, store)
	...
	re.NoError(failpoint.Disable("github.com/tikv/pdv9/server/versionChangeConcurrency"))
}
...
func TestSetScheduleOpt(t *testing.T) {
	...
	// PUT GET failed
	re.NoError(failpoint.Enable("github.com/tikv/pdv9/server/storage/kv/etcdSaveFailed", `return(true)`))
	...
	// DELETE failed
	re.NoError(failpoint.Disable("github.com/tikv/pdv9/server/storage/kv/etcdSaveFailed"))
	re.NoError(svr.SetReplicationConfig(*replicationCfg))
	re.NoError(failpoint.Enable("github.com/tikv/pdv9/server/storage/kv/etcdSaveFailed", `return(true)`))
...
// See https://github.com/tikv/pdv9/issues/4941
