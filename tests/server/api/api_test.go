...
func (suite *middlewareTestSuite) SetupSuite() {
	suite.NoError(failpoint.Enable("github.com/tikv/pdv9/server/api/enableFailpointAPI", "return(true)"))
	ctx, cancel := context.WithCancel(context.Background())
...
func (suite *middlewareTestSuite) TearDownSuite() {
	suite.NoError(failpoint.Disable("github.com/tikv/pdv9/server/api/enableFailpointAPI"))
	suite.cleanup()
	suite.cluster.Destroy()
}

func (suite *middlewareTestSuite) TestRequestInfoMiddleware() {
	suite.NoError(failpoint.Enable("github.com/tikv/pdv9/server/api/addRequestInfoMiddleware", "return(true)"))
	...
	suite.NoError(failpoint.Disable("github.com/tikv/pdv9/server/api/addRequestInfoMiddleware"))
}
